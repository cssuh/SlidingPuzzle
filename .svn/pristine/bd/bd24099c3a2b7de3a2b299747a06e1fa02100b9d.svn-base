...
public class SimplePuzzleStateTest {
	...
	@Test
	public void testFlipForTetrisShapes(){
		// set up state with 3 empty slots
		PuzzleState ps1 = new SimplePuzzleState();
		ps1.setToInitialState(4, 3);
		// 1 2 3 4
		...
		// 9 0 11 12
		// 13 0 0 10
		//move 6 to the left of 10
		PuzzleState ps5 = ps4.move(1, 1, Operation.MOVEDOWN);
		PuzzleState ps6 = ps5.move(2, 1, Operation.MOVEDOWN);
		PuzzleState ps7 = ps6.move(3, 1, Operation.MOVERIGHT);
		// 1 2 3 4
		// 5 0 7 8
		// 9 0 11 12
		// 13 0 6 10
		// move 7 from (1, 2) to (3, 1)
		PuzzleState ps8 = ps7.move(1, 2, Operation.MOVELEFT);
		PuzzleState ps9 = ps8.move(1, 1, Operation.MOVEDOWN);
		PuzzleState ps10 = ps9.move(2, 1, Operation.MOVEDOWN);
		PuzzleState test = ps7.flip(1, 2, 3, 1);
		// sequence and flip should match
		assertTrue("Comparing state " + ps10 + " with " + test, ps10.equals(test));
		// check if flip produce correct parent relationship with intermediate states
		PuzzleState parent = test.getParent();
		assertTrue("Comparing state " + ps9 + " with " + parent, ps9.equals(parent));
		parent = parent.getParent();
		assertTrue("Comparing state " + ps8 + " with " + parent, ps8.equals(parent));
		parent = parent.getParent();
		assertTrue("Comparing state " + ps7 + " with " + parent,ps7.equals(parent));
